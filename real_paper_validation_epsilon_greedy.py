#!/usr/bin/env python3
"""
Real Paper Validation with Proper Epsilon-Greedy Exploration

This script runs the real agent validation with proper 10% epsilon-greedy exploration
instead of forcing 100% exploration.
"""
...
logger = logging.getLogger("RealPaperValidationEpsilonGreedy")
...
class RealPaperValidationEpsilonGreedy:
    """Validation using original paper implementation with real agents and proper epsilon-greedy exploration."""
    
    def __init__(self):
        self.decision_engine = DecisionEngine()
        # Set a fixed random seed to ensure reproducible epsilon-greedy behavior
        random.seed(42)
    ...
    async def run_validation(self) -> Dict[str, Any]:
        """Run the real paper validation with proper epsilon-greedy exploration."""
        print("🔬 REAL PAPER VALIDATION WITH EPSILON-GREEDY EXPLORATION")
        print("=" * 65)
        print("Using original COALESCE implementation with real LLM agents")
        print("🎯 EPSILON-GREEDY: 10% exploration rate (proper implementation)")
        print("=" * 65)
        ...
        print(f"🎯 Exploration rate: 10% (epsilon-greedy)")
        
        # Execute tasks using original COALESCE decision engine with proper epsilon-greedy
        print("\n🔄 Executing tasks with proper epsilon-greedy exploration...")
        ...
        for i, task in enumerate(tasks, 1):
            ...
            try:
                # Use original COALESCE decision engine with proper epsilon-greedy
                ...
                if result['success']:
                    ...
                    savings_display = f"{decision.cost_savings:.1f}%" if decision.cost_savings >= 0 else f"{decision.cost_savings:.1f}% (loss)"
                    print(f"{exploration_marker} Task {i}: {result['agent']} - {savings_display} (TOPSIS: {decision.topsis_score:.3f})")
        ...
        results = {
            'validation_type': 'REAL_PAPER_IMPLEMENTATION_EPSILON_GREEDY',
        ...
        # Print summary
        print(f"\n📊 VALIDATION SUMMARY (EPSILON-GREEDY)")
...
async def main():
    ...
    validation = RealPaperValidationEpsilonGreedy()
    results = await validation.run_validation()
    
    # Save results with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"real_paper_validation_epsilon_greedy_{timestamp}.json"
